...
=======
/*  Created On : 06-Mar-2017 5:11:07 PM 				*/
>>>>>>> Stashed changes
/*  DBMS       : MySql 						*/
/* ---------------------------------------------------- */
...
<<<<<<< Updated upstream
=======
DROP TABLE IF EXISTS `media_user_answer` CASCADE
;
...
=======
	...
	`id_user_creator` INT NULL COMMENT 'Usuario Responsable de la Institución',
	`id_sector` INT NULL,
	`level_certified` TINYINT NOT NULL DEFAULT 0,
>>>>>>> Stashed changes
...
COMMENT = '    - Novato     - Intermedio     - Avanzado'
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
=======
CREATE TABLE `media_user_answer`
(
	`id_media` INT NULL,
	`id_user_answer` INT NULL
)
...
(
	`id` INT NOT NULL AUTO_INCREMENT,
	`name` VARCHAR(255) NULL,
	CONSTRAINT `PK_sector` PRIMARY KEY (`id` ASC)
)
>>>>>>> Stashed changes
...
ALTER TABLE `category` 
 ADD INDEX `IXFK_category_category` (`id_parent` ASC)
;
...
ALTER TABLE `evaluation_request` 
 ADD INDEX `IXFK_evaluation_request_institution` (`id_institution` ASC)
;

ALTER TABLE `institution` 
 ADD INDEX `IXFK_institution_sector` (`id_sector` ASC)
;

ALTER TABLE `media_user_answer` 
 ADD INDEX `IXFK_media_user_answer_media` (`id_media` ASC)
;

ALTER TABLE `media_user_answer` 
 ADD INDEX `IXFK_media_user_answer_user_answer` (`id_user_answer` ASC)
>>>>>>> Stashed changes
;
...
ALTER TABLE `category` 
 ADD CONSTRAINT `FK_category_category`
	FOREIGN KEY (`id_parent`) REFERENCES `category` (`id`) ON DELETE Restrict ON UPDATE Restrict
>>>>>>> Stashed changes
;
...
=======
ALTER TABLE `evaluation_request` 
 ADD CONSTRAINT `FK_evaluation_request_institution`
...
ALTER TABLE `institution` 
 ADD CONSTRAINT `FK_institution_sector`
	FOREIGN KEY (`id_sector`) REFERENCES `sector` (`id`) ON DELETE Restrict ON UPDATE Restrict
>>>>>>> Stashed changes
;
...
=======
ALTER TABLE `media_user_answer` 
 ADD CONSTRAINT `FK_media_user_answer_media`
	FOREIGN KEY (`id_media`) REFERENCES `media` (`id`) ON DELETE Restrict ON UPDATE Restrict
;

ALTER TABLE `media_user_answer` 
 ADD CONSTRAINT `FK_media_user_answer_user_answer`
	FOREIGN KEY (`id_user_answer`) REFERENCES `user_answer` (`id`) ON DELETE Restrict ON UPDATE Restrict
;
...
ALTER TABLE `requisite` 
 ADD CONSTRAINT `FK_question_form`
	FOREIGN KEY (`id_form`) REFERENCES `stages` (`id`) ON DELETE Restrict ON UPDATE Restrict
;
...
ALTER TABLE `user_answer` 
 ADD CONSTRAINT `FK_user_answer_questiontopic`
	FOREIGN KEY (`id_topic`) REFERENCES `questiontopic` (`id`) ON DELETE Restrict ON UPDATE Restrict
=======
	FOREIGN KEY (`id_question`) REFERENCES `requisite` (`id`) ON DELETE Restrict ON UPDATE Restrict
>>>>>>> Stashed changes
;
